...
export const SideMenu = forwardRef<HTMLDivElement, SideMenuProps>(
  function SideMenu(
    ...
    const [internalData, setInternalData] = useState<INode[] | undefined>(
      data || undefined,
    );
    ...
    const [selChild, setSelChild] = useState<string | undefined>(
      selectedChildItem,
    );
    ...
    useEffect(() => {
      ...
      internalData.forEach(
        (n) => (next[n.id] = n.id === activeId && !!n.childNodes?.length),
      );
      setExpandedMap(next);
    ...
    useEffect(() => {
      ...
      const next: Record<string, boolean> = {};
      internalData.forEach(
        (n) => (next[n.id] = n.id === selectedItem && !!n.childNodes?.length),
      );
      setExpandedMap(next);
    ...
    const setExpandedSingle = useCallback(
      (id?: string) => {
        setExpandedMap(() => {
          const next: Record<string, boolean> = {};
          (internalData || []).forEach(
            (n) => (next[n.id] = !!id && n.id === id && !!n.childNodes?.length),
          );
          return next;
        });
      },
    ...
    const renderBadge = useCallback(
      (badgeValue?: number, color?: any, outlined?: boolean) => {
    ...
    const Row = useMemo(() => {
      ...
      const C: React.FC<RowProps> = React.memo(
        ({ item, expanded, active }) => {
          const hasChild = !!item.childNodes && item.childNodes.length > 0;
          const targetHeightPx = hasChild
            ? item.childNodes!.length * rowHeightPx
            : 0;
          ...
          return (
            ...
            >
              <TitleContainer onClick={() => handleParentClick(item)}>
                <TitleText
                  className={`tx-title-text ${hasChild ? "has-child" : ""}`}
              ...
              {hasChild && (
                ...
                >
                  {item.childNodes!.map((c) => {
                    const controllerChild = isChildControlled
                      ? selectedChildItem
                      : selChild;
                    const cActive =
                      controllerChild === c.id || activeLocalId === c.id;
                    return (
                      <ChildRow
                        key={c.id}
                      ...
                      >
                        ...
                        <InfoContainer>
                          {typeof c.value === "number" ? (
                            <strong style={{ color: c.valueColor }}>
                              {c.value}
                            </strong>
                          ) : c.icon ? (
    ...
    if (!internalData || internalData.length === 0) {
      return (
        ...
        >
          <NoDataContainer className="fade-in-container">
            {noItemsText}
          </NoDataContainer>
        </Wrap>
      );
      ...
      >
        <MenuContainer className="menu-container">
          {internalData.map((item) => {
            const controllerActiveId = isItemControlled
              ? selectedItem
              : selItem;
            const active =
