...
export function summarizeFullWorkdesk(rows: FullRow[]) {
  ...
  const trend = delta === 0 ? "no change" : delta > 0 ? `+${delta}` : `${delta}`;

  const topProducts7 = countBy((r) => r.base.product, last7Start).slice(0, 3);
  const topLocations7 = countBy((r) => r.base.bookingLocation, last7Start).slice(0, 3);
  const status7 = countBy(
    (r) => r.derived?.status || (/Initiated/i.test(r.base.workflowStage) ? "PENDING" : "REGISTERED"),
    last7Start,
  );
  ...
  const list = (pairs: [string, number][]) =>
    pairs.length ? pairs.map(([k, n]) => `  - ${k}: **${n}**`).join("\n") : "  - (no data)";
...
export async function fetchWorkdeskFull(baseUrl = "http://localhost:4000"): Promise<WorkdeskFullRow[]> {
  const res = await fetch(`${baseUrl}/workdesk/full?limit=0`, { method: "GET" });
  if (!res.ok) throw new Error(`workdesk ${res.status}`);
  const payload = await res.json();
  return Array.isArray(payload?.rows) ? (payload.rows as WorkdeskFullRow[]) : [];
}
...
// Build a compact system message with terminology + snapshot + hotset
export function buildSystemMemoryMessage(args: {
  stats?: WorkdeskSummary | null;
  hotset?: HotsetItem[] | null;
  terminology?: typeof TERMINOLOGY | null;
}): { role: "system"; content: string } | null {
  const { stats, hotset, terminology } = args || {};
  const hs = Array.isArray(hotset) ? hotset.slice(0, 50) : [];
  if (!stats && hs.length === 0 && !terminology) return null;

  const payload = {
    terminology,
    snapshot: stats?.totals ?? null,
    hotset: hs,
    notes: {
      latestDefinition: "Latest = max __receivedAtMs / most recent receivedAt",
      hotsetLimit: 50,
    },
  };
  ...
  return {
    role: "system",
    content:
      "You are TradeExpress Helix Assistant. Treat each Workdesk row as a 'transaction'. " +
      "Use the memory JSON below to answer requests like 'latest transaction' or 'top 20 transactions'. " +
      "If a query requires more data than provided, ask for permission to search the backend.\n\n" +
      "MEMORY_JSON:\n```json\n" +
