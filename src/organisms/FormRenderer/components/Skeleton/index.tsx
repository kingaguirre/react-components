...
import { Description, SectionWrapper } from "../../styled";
import type {
  SettingsItem,
  FieldGroup,
  FieldSetting,
...
const SkeletonLabel = styled.div`
  ...
  margin-bottom: 8px;
  background: #eee;
  background-image: linear-gradient(to right, #eee 25%, #f5f5f5 50%, #eee 75%);
...
export function renderSkeletonSection(
  ...
  const renderDataTable = (table: DataTableSection, idx: number) => {
    ...
    const isHiddenTable =
      typeof hideTable === "function" ? hideTable(values) : hideTable === true;
