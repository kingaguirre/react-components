...
export const SideMenuItem: React.FC<SideMenuItemProps> = ({
  ...
  const handleMouseLeave = (e: React.MouseEvent) => {
    ...
    if (menuCollapsed) {
      ...
      const relatedMenuItem = (e.relatedTarget as HTMLElement)?.closest(
        ".side-menu-item",
      );
      if (relatedMenuItem) {
        if (hideTimeoutRef.current) {
  ...
  const getBadge = (badge) => {
    ...
    if (badge) {
      if (icon) {
        return (
          <Icon icon={icon} color={theme.colors[color ?? "primary"].base} />
        );
      }

      return (
        <Badge outlined={outlined} color={color as any}>
          {value ?? "?"}
        </Badge>
      );
    }
  ...
  return (
    ...
    >
      <MenuItemContent>
        <MenuItemWrapper
          className={`${isActive ? "active" : ""} ${item.children ? "has-child" : ""} ${menuCollapsed ? "collapsed" : ""}`}
        >
            ...
            (item.children || floating ? (
              isSubMenu ? (
                <MenuItemTitle $menuCollapsed={menuCollapsed}>
                  <OverflowTooltip>{item.title}</OverflowTooltip>{" "}
                  {!item.children ? getBadge(item.badge) : ""}
                </MenuItemTitle>
              ) : null
            ) : (
              <FloatingSubMenu $open={open}>
                <FloatingHeader>
                  {item.title} {getBadge(item.badge)}
                </FloatingHeader>
              </FloatingSubMenu>
            ))
          ) : (
            <MenuItemTitle $hasChild={!!item.children}>
              <OverflowTooltip>{item.title}</OverflowTooltip>{" "}
              {getBadge(item.badge)}
            </MenuItemTitle>
          )}
          {item.children && (
            <ArrowIconWrapper className={menuCollapsed ? "collapsed" : ""}>
              <Icon
                icon={
                  menuCollapsed ? "keyboard_arrow_right" : "keyboard_arrow_down"
                }
              />
            </ArrowIconWrapper>
          )}
