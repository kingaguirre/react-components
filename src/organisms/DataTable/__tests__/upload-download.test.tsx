...
describe('DataTable – Upload & Download', () => {
  ...
  test('Upload XLSX: issues → shows review, then onImport fires after Proceed', async () => {
    // Trigger review: include an invalid column and one empty row
    (globalThis as any).__mockUploadRows = [
      { ID: '10', Title: 'Soap', Brand: 'Clean Co', Category: 'beauty',    Price: '4.5', invalid_col: 'X' },
      { ID: '',   Title: '',     Brand: '',         Category: '',          Price: '',     invalid_col: '' }, // empty → ignored
      { ID: '11', Title: 'Desk', Brand: 'Oakline',  Category: 'furniture', Price: '199',  invalid_col: 'Y' },
    ];
    ...
    // Review modal should appear with warnings
    await screen.findByText(/import review/i);
    expect(screen.getByText(/some headers don’t match/i)).toBeInTheDocument();
    expect(screen.getByText(/empty rows will be ignored/i)).toBeInTheDocument();

    // Confirm import
    await u.click(screen.getByTestId('upload-confirm-button'));

    // Now onImport must fire with only the non-empty rows, aligned to visible cols
    ...
    // Modal should close after proceeding
    await waitFor(() => expect(screen.queryByText(/import review/i)).toBeNull());
  });

  test('Upload XLSX: no issues → skips review and imports immediately', async () => {
    // All headers match visible columns; no empty rows → no review modal
    (globalThis as any).__mockUploadRows = [
      { ID: '10', Title: 'Soap', Brand: 'Clean Co', Category: 'beauty',    Price: '4.5' },
      { ID: '11', Title: 'Desk', Brand: 'Oakline',  Category: 'furniture', Price: '199' },
    ];
    ...
    render(
      <DataTable
        ...
        columnSettings={DL_COLS}
        enableUpload
        uploadControls={{ title: 'Upload CSV/XLSX' }}
      />
    );
    ...
    // Review modal should never appear
    await waitFor(() => {
      expect(screen.queryByText(/import review/i)).toBeNull();
      expect(screen.queryByText(/some headers don’t match/i)).toBeNull();
      expect(screen.queryByText(/empty rows will be ignored/i)).toBeNull();
    });

    // Rows should render in the table immediately (DataTable prepends on import)
    await waitFor(() => {
  ...
  test('Upload XLSX: hidden column present → no unmatched headers warning', async () => {
    // DL_COLS includes a hidden 'Rating' column
    (globalThis as any).__mockUploadRows = [
      { ID: '10', Title: 'A', Brand: 'Acme', Category: 'beauty', Price: '10', Rating: '5' },
    ];

    render(
      <DataTable
        dataSource={[]}
        columnSettings={DL_COLS} // has hidden Rating
        enableUpload
        uploadControls={{ title: 'Upload CSV/XLSX' }}
      />
    );
    ...
    await u.upload(
      input,
      new File([new Uint8Array([0, 1, 2, 3])], 'ok.xlsx', {
        type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
      }),
    );

    // Should import immediately without "Some headers don’t match"
    await waitFor(() => {
      expect(screen.queryByText(/import review/i)).toBeNull();
      expect(screen.queryByText(/Some headers don’t match/i)).toBeNull();
    });
