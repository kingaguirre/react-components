...
describe('DataTable – Upload & Download', () => {
  ...
  //   // Grab the mocked .get and program it
  //   const axiosGet = vi.mocked((axios as any).get);
  //   axiosGet.mockResolvedValueOnce({ data: { products: PROD_ROWS } });

  //   render(
  //     <DataTable
  //       serverMode
  //       server={{ fetcher: async () => ({ rows: PROD_ROWS.slice(0, 2), total: PROD_ROWS.length }), debounceMs: 0 }}
  //       dataSource={[]}
  //       columnSettings={DL_COLS}
  //       enableDownload
  //       downloadControls={{
  ...
  //               // Import the same mocked axios the component sees
  //               const axiosMod = (await import('axios')).default as any;

  //               const { data } = await axiosMod.get('https://dummyjson.com/products', {
  //                 params: { limit: 0, select: 'id,title,brand,category,price,rating' },
  //               });
  //               const rows = Array.isArray(data?.products) ? data.products : [];

  //               // Build AOA
  //               const header = DL_COLS.map((c) => c.title);
  //               const body = rows.map((r) => DL_COLS.map((c) => (r as any)[c.column]));
  //               const aoa = [header, ...body];

  //               // Use mocked xlsx to record __lastAOA
  //               const XLSX: any = await import('xlsx');
  //               const ws = XLSX.utils.aoa_to_sheet(aoa);
  //               const wb = XLSX.utils.book_new();
  //               XLSX.utils.book_append_sheet(wb, ws, 'Data');
  //               XLSX.write(wb, { bookType: format, type: 'array' });
  //             },
  //           },
  //         ],
  //       }}
  ...
  //   await openDownloadMenu();
  //   await userEvent.click(screen.getByRole('menuitem', { name: /download all from server/i }));
  ...
  //   expect(globalThis.__lastAOA).toHaveLength(PROD_ROWS.length + 1);
  // });
