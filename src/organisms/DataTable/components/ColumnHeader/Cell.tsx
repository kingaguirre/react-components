...
export const Cell = ({ header, table, enableColumnDragging }: {
  ...
  const { attributes, isDragging, listeners, setNodeRef, transform } = useSortable({ id: colDef.accessorKey })
  const isCustomColumn = CUSTOM_COLUMN.includes(header.column.id)
  const hasDND = enableColumnDragging && (colMeta?.draggable !== false && !isCustomColumn)
  const hasPin = header.column.getCanPin() && !isCustomColumn
  const hasSort = header.column.getCanSort() && !isCustomColumn
  const hasFilterableColumn = table.getAllLeafColumns().some(column => column.getCanFilter())
  ...
  const cellPlaceholder = (header.isPlaceholder || isGroupHeader) ? null : <CellFilterPlaceholder />
  const columnFilter = header.column.getCanFilter() ? <Filter column={header.column} /> : cellPlaceholder
      ...
      {header.isPlaceholder ? null : (
        <>
          ...
          {(hasPin || hasSort) && !isGroupHeader && (
            <LeftIconContainer data-testid={`left-icon-container-${header.column.id}`}>
              {/** Pinning */}
              {hasPin && (
                <IconContainer
                ...
                >
                  <Tooltip type='title' content={`${isPinLeft ? 'Unpin' : 'Pin'} ${headerText}`}>
                    <Icon icon='push_pin' />
                  </Tooltip>
                </IconContainer>
              )}
