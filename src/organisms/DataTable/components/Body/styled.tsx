...
export const CellContainer = styled.div<{
  ...
  ${({ $isEditMode, $isPinned }) =>
    $isEditMode ? `z-index: ${ifElse(!!$isPinned, 14, 10)}!important;` : ""}
...
export const NoDataContainer = styled.div<{ $hasError?: boolean }>`
  ...
  font-weight: bold;
  color: ${({ $hasError }) =>
    $hasError ? theme.colors.danger.base : theme.colors.default.base};
  text-shadow:
    0 1px 1px white,
    1px 0px 1px white,
    0px -1px 1px white,
...
export const ExpandedRowContainer = styled.div<{
  ...
  font-size: 14px;
  width: calc(
    100% + ${({ $expandedRowRightOffset }) => $expandedRowRightOffset ?? 0}px
  );
`;
