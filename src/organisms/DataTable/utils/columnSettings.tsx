...
export const CUSTOM_COLUMN = [ DATA_TABLE_SELECT_ID, DATA_TABLE_EXPANDER_ID, DATA_TABLE_ROW_ACTION_ID ]
...
export const getInitialSorting = (columnSettings: ColumnSetting[]): SortingColumn[] => columnSettings
...
export const getInitialPinning = (columnSettings: ColumnSetting[]): ColumnPinningType => {
...
export const getInitialColumnOrder = (columns: ColumnWithMeta[]): string[] => {
  ...
  const getKey = (col: ColumnWithMeta): string => col.accessorKey ?? col.id ?? ''
  ...
  flatColumns.forEach(col => {
    if (col.meta?.className !== 'custom-column') {
      ...
      if (col.meta && col.meta.order != null && !isNaN(parseFloat(col.meta.order.toString()))) {
        nonCustomColumns.push({ col, order: parseFloat(col.meta.order.toString()) })
      } else {
  ...
  const unorderedNonCustom = nonCustomColumns.filter(item => item.order === undefined)
