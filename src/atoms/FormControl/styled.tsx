...
export const CustomCheckboxRadio = styled.input<{
  ...
  &:before {
    ...
    position: relative;
    transform: scale(0);
    ${({ type, color = "primary", size }) =>
      getCustomCheckboxRadio({ type, color, size, _theme: theme })}
  }

  &:focus:not(:disabled) {
    outline: none;
    box-shadow: ${({ color = "primary" }) =>
      `0 0 0 4px ${theme.colors[color].pale}, 0 0 1px 5px ${theme.colors[color].lighter}`};
  }
...
export const Text = styled.span<{
  ...
  font-family: ${theme.fontFamily};
  color: ${({ disabled }) =>
    `${theme.colors.default[disabled ? "base" : "dark"]}`};
  line-height: 1.2;
  pointer-events: none;
  text-align: left;
`;
...
export const GroupControlContainer = styled.div<{
  ...
  gap: ${({ $gap = 12 }) => $gap}px;
  ${({ $isVerticalOptions }) =>
    $isVerticalOptions ? "flex-direction: column;" : ""}
`;
